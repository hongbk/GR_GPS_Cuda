...
int main()
{
	...
	char navfn[256]="F:\\GR\\GR_Cuda_GPS\\brdc3540.14n";
	char user_motion[256]="F:\\GR\\GR_Cuda_GPS\\circle1.csv";
	char nmea_gga[256] = { 0 };
	...
	if (isEmpty(frequecy) == 0) {
		samp_freq = atof(frequecy);
		if (samp_freq < 1.0e6)
		{
	...
	if (isEmpty(datetime) == 0) {
		sscanf(datetime, "%d/%d/%d,%d:%d:%lf", &t0.y, &t0.m, &t0.d, &t0.hh, &t0.mm, &t0.sec);
	...
	*/
	...
	if (data_format == SC08)
	{
		iq8_buff = calloc(2 * iq_buff_size, 2);
	...
	for (iumd = 1; iumd < numd ; iumd++)
	{
		...
		/*for (isamp = 0; isamp < iq_buff_size; isamp++)
		{
			int i_acc = 0;
			int q_acc = 0;
			//i_acc = (arrayCos[isamp] + 50) / 100;
			//q_acc = (arraySin[isamp] + 50) / 100;
			i_acc = (arrayCos[isamp] + 64) >> 7;
			q_acc = (arraySin[isamp] + 64) >> 7;
			iq_buff[isamp * 2] = (short)i_acc;
			iq_buff[isamp * 2 + 1] = (short)q_acc;
		}
		/*for (i = 0; i < MAX_CHAN; i++)
